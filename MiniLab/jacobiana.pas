      ...
      begin
        ...
        for j:=0 to Jelementos-1  do
          begin
            ...
            //ShowMessage(Jfunciones[0,i]+'  &  '+Jvariables[0,j]+ '  &  '+ FloatToStr(Jvalores[0,j]) );
            help[j,i]:=derivadaParcial(m_function,Jvariables[0,j],Jvalores[0,j]);
            //ShowMessage(floatToStr(help[i,j]));
          end;
        //ShowMessage('endl');
      end;
  Evaluate:=help;
...
begin
  ...
  funaux.NewValue(x,v);
  b:=funaux.Evaluate();
  //ShowMessage( 'evaluando ' + FloatToStr(b));
  funaux.NewValue(x,v+h);
  a:=funaux.Evaluate();
  //ShowMessage( 'evaluando2 ' + FloatToStr(a));
